	...
	private static class ReferenceScanner extends ElementScanner8<Void, Void> {
		...
		public Void visitVariable(VariableElement e, Void p) {
			...
			if (varType.getKind().equals(TypeKind.DECLARED)) {
				...
				final AccessElement referencedElement = AccessElement.type(types.erasure(varType).toString());
				mTypes.add(referencedElement);
			}
		...
		public Void visitType(TypeElement e, Void p) {
			...
			final Types types = mEnv.getTypeUtils();
			mTypes.add(AccessElement.type(e.getSuperclass().toString()));
			for (TypeMirror implInterface : e.getInterfaces()) {
				...
				mTypes.add(AccessElement.type(erased.toString()));
			}
		...
		public Void visitTypeParameter(TypeParameterElement e, Void p) {
			for (TypeMirror typeBound : e.getBounds()) {
				mTypes.add(AccessElement.type(typeBound.toString()));
			}
